...
\begin{figure}[H]
    ...
    \caption{\ac{snn} structure with encoding and decoding layers. Each sub-layer consists of a fuzzy encoder and the F2S Converter, with the output layer receiving inputs from synaptic weights and a random action selector. During the training phase, the output layer receives input only from the random action selector, which then shifts to synaptic weight inputs after the training.}
...
\noindent where \(\mathcal{R}_{max}^{\mathcal{S}}\) is the maximum reward in the network section (e.g., \(max(\mathcal{R}_{Fi}^{Fj})\)), \(\mathcal{R}_{max}^{G} = max(\mathcal{R}_{Fi}^{Fj}, \mathcal{R}_{Fi}^{L})\), and \(\epsilon\) is a small positive number that controls the steep of the function around \(W_{max}^{\mathcal{S}}(t) =  \Psi^{\mathcal{S}}\). This model determines the learning rate by the highest synaptic weight among the active input and output neurons. This mechanism is similar to the ``winner-takes-all'' approach. When a synaptic connection reaches its weight limit, it prevents further changes in the adjacent synaptic weights. 
...
\noindent \noindent where \(\eta = \frac{\mathcal{A}\mathcal{R}_{max}^{G} \ln{(2^{\lambda^2}-1)}}{\lambda\Psi^{\mathcal{S}} \log{(2)}}\) is a scaling parameter that can adjust the output scale of the function, and \(\beta = \frac{\ln{(2^{\lambda^2}-1)}}{\Psi^{\mathcal{S}}}\) controls the curvature of the function. A higher 
\(\beta\) makes the SoftPlus function approach a step function, making it closer to the binary behavior. Conversely, a smaller \(\beta\) makes the function smoother and more gradual. Equation \eqref{Eq.WS.3} can be represented as,
...
% In this set, neurons 15, 16, 17, and 18 are fired input neurons, and neuron 73 is the fired output neuron. First, the algorithm finds the maximum synaptic weight connecting these input neurons to the output neuron, denoted as \(W_{max}^{\mathcal{S}}(t)\). Let's assume that \(W_{max}^{\mathcal{S}}(t) = 7\), and \(
% \mathcal{R}_{max}^{\mathcal{S}} = 0.5, \quad \mathcal{R}_{max}^{G} = 1, \quad I^{max} = 15.5,\) that can be calculated based on the neuron model and the reward function characteristics.

% We use these values to find the learning rate \(\boldsymbol{\alpha}^{\mathcal{S}}(t)\) for the synaptic weights.  Substituting the given values:
% \[
% \Psi^{\mathcal{S}} = \frac{0.5}{1} \times 15.5 = 7.75
% \]

% Assuming \(\epsilon\) is a small positive number (e.g., 0.01), the learning rate for these synaptic weights \(\boldsymbol{\alpha}^{\mathcal{S}}(t)\) can be calculated as:
% \[
% \boldsymbol{\alpha}^{\mathcal{S}}(t) = \frac{1}{1 + \exp{\left[\frac{1}{0.01}\left( 7 - 7.75 \right)\right]}} = \frac{1}{1 + \exp{\left[-75\right]}}
% \]

% Given that \(\exp{\left[-75\right]}\) is very close to 0, \(\boldsymbol{\alpha}^{\mathcal{S}}(t)\) approaches 1, indicating a high learning rate for synaptic adjustment.

% When the maximum synaptic weight \(W_{max}^{\mathcal{S}}(t)\) surpasses the value of \(\Psi^{\mathcal{S}}\) (e.g., \(W_{max}^{\mathcal{S}}(t) = 8\)), the learning rate \(\boldsymbol{\alpha}^{\mathcal{S}}(t)\) changes as follows:

% \[
% \boldsymbol{\alpha}^{\mathcal{S}}(t) = \frac{1}{1 + \exp{\left[\frac{1}{0.01}\left( 8 - 7.75 \right)\right]}} = 0.0759
% \]

% This means that there is a negligible adjustment rate for the synaptic weights. Therefore, the maximum synaptic weight in \(\mathcal{S}\) prevents further growth, maintaining stability in the network.

% When the reward changes such that \(\mathcal{R}_{max}^{\mathcal{S}} = \mathcal{R}_{max}^{G}\), the learning rate changes from 0 to 1, causing the weights to start growing again. This happens because \(\Psi^{\mathcal{S}}\) changes as follows:

% Given:
% \[
% \mathcal{R}_{max}^{\mathcal{S}} = \mathcal{R}_{max}^{G} = 1, \quad I^{max} = 15.5, \quad W_{max}^{\mathcal{S}}(t) = 8
% \]

% Substituting into \(\Psi^{\mathcal{S}}\):
% \[
% \Psi^{\mathcal{S}} = \frac{\mathcal{R}_{max}^{\mathcal{S}}}{\mathcal{R}_{max}^{G}} I^{max} = \frac{1}{1} \times 15.5 = 15.5
% \]

% Now, calculating the learning rate:
% \[
% \boldsymbol{\alpha}^{\mathcal{S}}(t) = \frac{1}{1 + \exp{\left[\frac{1}{0.01}\left( 8 - 15.5 \right)\right]}} = \frac{1}{1 + \exp{\left[-750\right]}}
% \]

% Given that \(\exp{\left[-750\right]}\) is extremely close to 0, \(\boldsymbol{\alpha}^{\mathcal{S}}(t)\) approaches 1, indicating a very high learning rate for synaptic adjustment, allowing the weights to grow significantly.
...
In this set, neurons 15, 16, 17, and 18 are fired input neurons, and neuron 73 is the fired output neuron. First, the algorithm finds the maximum synaptic weight connecting these input neurons to the output neuron, denoted as \(W_{max}^{\mathcal{S}}(t)\). Let's assume that \(I^{max} = 15.5\) and \(\epsilon\) is a small positive number (e.g., 0.01).
...
\begin{equation*}
    \boldsymbol{\alpha}^{\mathcal{S}}(t) = \frac{1}{1 + \exp{\left[\frac{1}{0.01}\left(7 - 7.75\right)\right]}} = 1
\end{equation*}

The decay rate is low enough in comparison with the learning rate to allow for synaptic growth (\(\mathcal{A}\mathcal{R}_{max}^{\mathcal{S}}\)), promoting an increase in synaptic strength.

\textbf{Case 2: \(W_{max}^{\mathcal{S}}(t) > \Psi^{\mathcal{S}}\)}

Let's assume that \(W_{max}^{\mathcal{S}}(t) = 10\), the decay rate \(\Theta^{\mathcal{S}}\) can be calculated as follows,
...
\begin{equation*}
    \boldsymbol{\alpha}^{\mathcal{S}}(t) = \frac{1}{1 + \exp{\left[\frac{1}{0.01}\left(10 - 7.75\right)\right]}} \approx 0
\end{equation*}

This results in a higher decay rate for the set \(\mathcal{S}\) (while \(\alpha^{\mathcal{S}}=0\)), actively working to reduce synaptic strength towards the threshold, due to \(W_{max}^{\mathcal{S}}(t)\) exceeding \(\Psi^{\mathcal{S}}\).

\textbf{Case 3: When \(\Psi^{\mathcal{S}}\) increases from 7.75 to 15.5 (Reward Change)}

Assuming a reward change causes \(\Psi^{\mathcal{S}}\) to increase to 15.5 (\(\mathcal{R}_{max}^{\mathcal{S}}=\mathcal{R}_{max}^{G}=1\)) and considering \(W_{max}^{\mathcal{S}}(t) = 7.8\), the decay rate \(\Theta^{\mathcal{S}}\) is recalculated as follows,
...
\end{proof}
    ...
    \begin{figure}[hb]
        ...
        \begin{minipage}{0.48\textwidth}
            \includegraphics[width=\linewidth] {Figures/RCSE.pdf}
            \caption{Synaptic weight change for $\lambda=5, \Psi^{\mathcal{S}}=15.5$,\\ and $\mathcal{A R}^G=1$.}
            \label{fig_3_3_1}
        \end{minipage}
        \begin{minipage}{0.51\textwidth}
            \includegraphics[width=\linewidth] {Figures/RCSE Principle.pdf}
            \vspace{5ex}
            \caption{RCSE working principle in inhibiting the adjacent synaptic connections.}
            \label{fig_3_3_2}
        \end{minipage}
    \end{figure}
    
    Figure \ref{fig_3_3_1} demonstrates the performance of the \ac{RCSE} when it regulates the synaptic weights to prevent unbounded growth. The red dotted line represents the value of \(\Psi^{\mathcal{S}}\), which is derived from the maximum reward value of the corresponding network section. As shown in Figure \ref{fig_3_3_1}, the synaptic weight oscillates around \(\Psi^{\mathcal{S}}\), and when it drops below \(\Psi^{\mathcal{S}}\), the learning rate is set to 1 by \eqref{Eq.WS.1}. This allows any changes in the reward function to be applied to the synapse. Figure \ref{fig_3_3_2} shows how the maximum synaptic weight of the active synapses in set \(\mathcal{S}\) stops the adjacent synaptic connections' growth by setting the learning rate of the set to 0.
...
The \ac{stdp} mechanism updated synaptic weights at 10 ms intervals. During these intervals, the learning algorithm adjusted the agent's states based on received data from other agents and the leader while simultaneously generating random outputs as part of an exploration strategy (section 3.2.3).
...
As mentioned in section 3, the input encoding uses the error between current and commanded distance. Therefore, one of the advantages of the encoding and learning method in this paper is that the learned policies are independent of the commanded distance. The commanded distance can be changed after training since the \ac{snn} uses the distance error. Figure \ref{fig_3_2} shows the agents' response to changes in commanded distance after training. According to this figure, when the commanded distance is changed at 30 seconds, the swarm immediately responds to this change in 2.98 seconds without disrupting the formation or any collision.
...
The simulation proceeded for an additional 1200 seconds, during which the synaptic weights were adjusted in accordance with the new reward function given by \eqref{Eq.R-1}. In this case, because the leader is now an obstacle, so then \(\alpha^{\mathcal{S}}(t)\) in Fig. 2.4 switches to zero and training starts again with random signals to the output layer.

The results of the reward change are shown in Figures \ref{fig_3_6} and \ref{fig_3_7}, which indicates that the agents quickly reduced their initial distance to the commanded distance of 2 m. Simultaneously, the minimum distance from the obstacle, which was the leader, increased over time, indicating that the agents adapted their behavior to maintain a greater distance from the obstacle. Figure \ref{fig_3_7} shows the trajectory of each agent after the reward change.
...
\begin{table}[H]
    ...
    \hline
    \textbf{Learning Time - Training Phase (s)}                & 261.92               & 554.71\\
...
Our model's dynamic nature of the model aggregation time adjusts based on the Euclidean norm. This metric measures the distance between the weight matrices of the agents and the server, determining reduced intervals for model publication. Our results show that the proposed aggregation method significantly accelerates agents' learning while increasing the accuracy of the swarm in following the commanded distance.
