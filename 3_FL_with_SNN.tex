...
\noindent where \(\mathcal{q}_{t}\) is the random action at time \( t \), \( \gamma \) is a correlation coefficient, and \( \Upsilon_{t} \) is a random number drawn from a standard distribution (e.g., Gaussian) at time \( t \). This equation ensures that the random action at any given time \( t \) is a weighted combination of the previous action and a new random number. This approach is a first-order filter, and the output is a colored noise.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        ...
        \caption{Distances during the test phase (\(\boldsymbol{\alpha}^{\mathcal{S}}=0\)) before reward change in the proposed event-triggered \ac{fl} method.}
        \label{fig_3-7}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \includegraphics[width=\linewidth]{Figures/Distances - ARC - DAI.pdf}
        \caption{Distances during the test phase (\(\boldsymbol{\alpha}^{\mathcal{S}}=0\)) after reward change in the proposed event-triggered \ac{fl} method.}
        \label{fig_3-8}
    \end{minipage}
\end{figure}
