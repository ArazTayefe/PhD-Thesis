...
% In the ATD, a defender tries to protect a target, while a superior invader with higher velocity than the defender and a moving target tries to reach the target and escape the defender at the same time. In the context of defense strategy, aircraft movement analysis is crucial in determining the most effective strategies. Law enforcement benefits from these games by minimizing escape possibilities. The ATD problems are a fundamental component of game theory, providing insights into strategic decision-making across disciplines like economics and biology. Additionally, these games find applications in Cybersecurity for modeling attacker-defender interactions \cite{Ch5_ATD4-5}.


The Target--Attacker--Defender (ATD) problem is formulated as a differential game involving three agents: the Target (T), the Attacker (A), and the Defender (D). Each agent's state is defined by its planar position \((x_i, y_i)\), with dynamics given by \cite{Ch5_ATD4-5}:
\begin{equation}
    \dot{x}_i = v_i \cos \theta_i, \quad \dot{y}_i = v_i \sin \theta_i, \quad i \in \{A, D, T\}
\end{equation}

\noindent where \(v_i \in [0, \bar{v}_i]\) is the speed and \(\theta_i\) is the heading angle of agent \(i\). Defining the speed ratios \(\alpha = \frac{\bar{v}_T}{\bar{v}_A}\) and \(\gamma = \frac{\bar{v}_D}{\bar{v}_A}\), the superior attacker case satisfies \(0 < \alpha < \gamma < 1\), where the Attacker is faster than both the Target and the Defender \cite{Ch5_ATD4-5}.

The game evolves based on the distances
\begin{equation}
    d = \sqrt{(x_A - x_D)^2 + (y_A - y_D)^2}, \quad \delta = \sqrt{(x_A - x_T)^2 + (y_A - y_T)^2}
\end{equation}

\noindent and terminates under the capture conditions: the Attacker wins if \(\delta < \rho_A\), while the Defender succeeds if \(d < \rho\), where \(\rho_A > \rho > 0\) denote the capture radii \cite{Ch5_ATD4-5}.

The reachable regions that define the interaction between agents are characterized using Cartesian ovals. In particular, for the Attacker--Defender interaction (Type-E), the boundary of the reachable set is given by \cite{Ch5_ATD4-5}
\begin{equation}
    x = x_A + r(\phi)\cos(\lambda + \phi + \pi), \quad
    y = y_A + r(\phi)\sin(\lambda + \phi + \pi)
\end{equation}

\begin{equation}
    r(\phi) = \frac{\gamma \rho + d\cos\phi \pm \sqrt{(\gamma \rho + d\cos\phi)^2 - (1-\gamma^2)(d^2 - \rho^2)}}{1 - \gamma^2}
\end{equation}

\noindent where \(\lambda\) is the line-of-sight angle from the Defender to the Attacker and \(\phi\) parameterizes the boundary. This Cartesian Oval (Type-E) separates the regions where the Defender can intercept the Attacker from those where the Attacker can evade, forming the basis for optimal strategy design.
...
In this figure, the LOS angles are explicitly defined for each agent: \(\phi_I^T\) and \(\phi_I^D\) denote the angles from the Invader to the Target and Defender, respectively, while \(\phi_D^T\) and \(\phi_D^I\) denote the angles from the Defender to the Target and Invader. All angles are measured with respect to the global \(X\)-axis.
...
In the \ac{snn}, pre-synaptic neurons correspond to input neurons, while post-synaptic neurons correspond to output neurons. Synaptic plasticity is governed by an eligibility trace, denoted by $C^{i\rightarrow j}$, which accumulates spike-timing correlations and decays over time. The eligibility trace evolves according to~\cite{Ch_1_R22}


\begin{equation} \label{Eq.14}
    \dot{C}^{i\rightarrow j}(t)
    =
    -\frac{C^{i\rightarrow j}(t)}{\tau_C}
    +
    \sum_{k}\sum_{\ell}
    \text{STDP}^{i\rightarrow j}\!\left(t_{\text{post}}^{(\ell)} - t_{\text{pre}}^{(k)}\right)
    \,\delta\!\left(t - t_{\text{post}}^{(\ell)}\right),
\end{equation}


\noindent

where \(t_{\text{pre}}^{(k)}\) and \(t_{\text{post}}^{(\ell)}\) denote the \(k\)-th pre-synaptic and \(\ell\)-th post-synaptic spike times, respectively. The summations are taken over all spike events of the corresponding neurons. The $C^{i\rightarrow j}(t)$ is the eligibility trace associated with the synapse from pre-synaptic neuron $i$ to post-synaptic neuron $j$, and $\tau_C$ is the eligibility decay time constant. The variables $t_{\text{pre}}$ and $t_{\text{post}}$ denote the firing times of the pre- and post-synaptic neurons, respectively. The Dirac delta function $\delta(t - t_{\text{post}})$ ensures that the eligibility trace is updated only at post-synaptic spike times, while the magnitude of the update depends on the relative spike timing $t_{\text{post}} - t_{\text{pre}}$ through the \ac{stdp} kernel.

The spike-timing-dependent plasticity function is defined as

\begin{equation} \label{Eq.15}
    \text{STDP}^{i\rightarrow j}(\tau) =
    \begin{cases}
        A_{+}\exp\!\left(-\dfrac{\tau}{\tau_s}\right), & \tau > 0, \\
        A_{-}\exp\!\left(\dfrac{\tau}{\tau_s}\right),  & \tau < 0,
    \end{cases}
\end{equation}

\noindent
where $\tau = t_{\text{post}} - t_{\text{pre}}$ is the spike timing difference, $A_{+}$ and $A_{-}$ are the amplitudes of potentiation and depression, respectively, and $\tau_s$ is the \ac{stdp} time constant that determines the temporal sensitivity of synaptic modification.

The parameter $\tau_s$ controls how strongly synaptic updates depend on precise spike timing. For small $\tau_s$, the exponential terms decay rapidly, and only closely timed pre- and post-synaptic spikes contribute significantly to the eligibility trace. As $\tau_s$ increases, the temporal window of interaction widens, allowing spikes separated by larger time intervals to influence synaptic plasticity. In the limiting case $\tau_s \rightarrow \infty$, the exponential terms converge to unity, and the STDP function becomes independent of spike timing. In this regime, synaptic updates no longer encode temporal causality and reduce to constant-amplitude contributions at each post-synaptic spike, eliminating the temporal selectivity that characterizes spike-timing-dependent plasticity.
...
The \ac{stdp} time constant $\tau_s$ is selected in relation to the synaptic weight update interval $\Delta T$. The objective is to ensure that spike pairs occurring anywhere within the update interval produce a meaningful contributions to the eligibility trace. To this end, $\tau_s$ is chosen such that the exponential \ac{stdp} kernel decays substantially, but not completely, over the duration of $\Delta T$. In this chapter, the weight update interval is set to $\Delta T = 10~\mathrm{ms}$, and $\tau_s$ is selected as $\tau_s = 3~\mathrm{ms}$. With this choice, when pre- and post-synaptic neurons fire close in time near the beginning of the update interval, the resulting eligibility trace retains a nonzero value when the time reaches the end of the interval, allowing the subsequent reward signal to modulate the corresponding synaptic weight.



This chapter uses the Multiplicative Synaptic Normalization (MSN) method to keep runaway excitation under control. Intuitively, MSN enforces a fixed total synaptic input budget for each post-synaptic neuron, such that an increase in one synaptic weight must be compensated by a proportional decrease in others. This preserves the relative structure of the learned weights while preventing unbounded growth. This method keeps pre-existing memories in the network by conserving the proportional difference between smaller and larger synaptic weights. According to (\ref{Eq.13}), we can calculate the maximum input for each post-synaptic neuron (output layer).
...
\begin{equation} \label{Eq.17}
    \overrightarrow{W}^n(t) = \overrightarrow{W}^n(t-1) \left(\frac{{I}^{max}}{\sum_{\iota=1}^{N} W^\iota(t)}\right)
\end{equation}

\noindent where $\overrightarrow{W}^n(t)$ is the vector consisting of synaptic weights that send input current to the $n^{th}$ output neuron, and $N$ is the total number of input synapses for each output neuron. The normalization factor rescales all incoming weights such that their sum equals $I^{max}$, ensuring that the total excitation received by neuron $n$ remains bounded. Therefore, when \ac{stdp} increases a single synaptic weight, the MSN proportionally decreases the other synaptic weights for the $n^{th}$ neuron.
...
Figure \ref{fig:my_label1} illustrates the defender’s network structure and encoding process. The invader employs the same network architecture with different inputs. The network receives the line-of-sight (LOS) angles and converts them into fuzzy membership values (FMVs) using \ac{grf}~\cite{Ch5_SNNModel1}. Since the fuzzy membership values are used solely for encoding continuous inputs into the spiking neural network, the specific choice of the membership function does not affect the computational complexity of the model.

Let $x \in \mathbb{R}$ denote an input variable (e.g., a LOS angle). The Gaussian receptive field associated with the $i$-th input neuron is defined as

\begin{equation}
\mu_i(x) = \exp\!\left( -\frac{(x - c_i)^2}{2\sigma_i^2} \right),
\end{equation}
where $c_i$ is the center of the receptive field and $\sigma_i$ controls its width. Each input neuron is assigned a distinct receptive field, and the corresponding membership value $\mu_i(x) \in [0,1]$ represents the degree to which the input activates that neuron.

There are $q$ input neurons, and a membership function is assigned to each neuron. Therefore, there are $q$ membership functions. The acquired fuzzy membership values are real numbers between 0 and 1 and are converted into input currents.. This can be done using a linear function and the minimum and maximum inputs.


\begin{figure}[H]
    \centering
    \includegraphics{Figures/4.pdf}
    \caption{Network structure and encoding process for the input layer (Defender). Each neuron is associated with a membership function in \ac{grf}. The \ac{grf} encodes an input State ($S^t$) at each time step. There is both a training phase when ($\xi  = 0$) and an operating phase when ($\xi = 1)$. The ``F2S Conversion" block converts fuzzy membership values to currents for the neurons according to \eqref{Eq.25-1}.}
    \label{fig:my_label1}
\end{figure}
...
\noindent
where \(I_{\sigma}\), \(I^{min}\), and \(I^{max}\) are currents measured in amperes (A). The membrane potential terms \(V_{th}\), \(V_{res}\), and \(E_l\) are expressed in volts (V), the membrane resistance \(R_m\) is in ohms (\(\Omega\)), and the time constants \(\tau_m\) and \(\Delta t\) are in seconds (s), ensuring dimensional consistency of the current expression.
...
\begin{algorithm}
        ...
        \vspace{10pt}
        \State{$I_{out} =  \xi I_{syn}+(1-\xi)I_{rnd}$ ($\xi  = 0$ in training phase)}
        \vspace{10pt}
...
After training, the $\xi$ changes to 1 and eliminates the effect of random output, and the \ac{snn}'s output is calculated based on the synaptic currents. Algorithm \ref{Algorithm4.1} shows the training process.
...
In Figure \ref{fig:my_label12}, we observe the performance of two methods, namely the \ac{snn} and the Cartesian Oval (CO) method, in the presence of noise. The CO method computes the capture location by explicitly constructing the boundary that separates the reachable regions of the attacker and the defender under constant-speed motion. This boundary is defined as a CO, which represents the locus of points for which the time-to-go of both players is equal. Let $(x_A,y_A)$ denote the attacker position, $d$ the distance between the attacker and defender, $\rho$ the initial distance between the attacker and the target, and $\gamma = v_D / v_A$ the defender-to-attacker speed ratio. The capture boundary is parameterized as
\begin{equation}
x = x_A + r(\phi)\cos(\lambda + \phi + \pi), \quad
y = y_A + r(\phi)\sin(\lambda + \phi + \pi),
\end{equation}
where $\lambda$ is the line-of-sight angle from the attacker to the defender and $\phi$ is a free angular parameter. The radial function $r(\phi)$ is given by
\begin{equation}
r(\phi) =
\frac{\gamma \rho + d \cos\phi \pm
\sqrt{(\gamma \rho + d \cos\phi)^2 - (1-\gamma^2)(d^2 - \rho^2)}}
{1-\gamma^2},
\end{equation}
with $\phi$ constrained to the interval $\phi \in [-\phi_l,\phi_l]$, where
\begin{equation}
\phi_l =
\arccos\!\left(
\frac{\sqrt{(1-\gamma^2)(d^2-\rho^2)} - \gamma \rho}{d}
\right).
\end{equation}
The capture location is determined by evaluating this parametric curve and selecting the point on the oval that satisfies the game termination condition, i.e., equal arrival time of the attacker and defender to the target. Since the oval is constructed directly from instantaneous position measurements, any perturbation in the measured states affects the shape and position of the oval, making the resulting capture point sensitive to measurement noise.

The noise in this experiment is introduced as white Gaussian noise, characterized by a mean of zero and a variance of 0.01. Both \ac{snn} and CO receive position data that has been corrupted by this noise. To ensure statistical reliability of the results, the simulation under noisy conditions has been repeated ten times, and the trajectories shown in Figure \ref{fig:my_label12} correspond to the average performance over these runs.
