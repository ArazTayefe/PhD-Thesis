...
In multiagent systems, one of the main strategies for sharing knowledge across distributed agents is \ac{fl}. The \ac{fl} is a decentralized training framework where agents exchange model updates instead of raw data. In this thesis, \ac{fl} is integrated with spiking neural networks to enable collaborative learning across multiple event-driven agents, allowing each deep SNN to adapt locally via R-STDP while periodically contributing its learned synaptic structure to a global model without violating bandwidth or energy constraints.

Although federated learning (\ac{fl}) is not the primary focus of this thesis, its principles provide an important complementary framework for scaling \ac{snn}-based systems. \ac{fl} enables distributed agents to train collaboratively without sharing raw sensory data, preserving privacy and reducing communication loads—qualities particularly relevant for energy-constrained robotic platforms. When combined with the sparse, event-driven nature of \ac{snn}s, \ac{fl} can reduce bandwidth requirements and support decentralized learning across heterogeneous devices \cite{Ch_1_R4, Ch_1_R5}.

Integrating \ac{snn}s into federated workflows introduces unique challenges, including mismatched timescales between local spiking updates and global aggregation, non-stationary input statistics, and communication–plasticity trade-offs. Only the final part of this thesis addresses these issues by exploring how reward-modulated deep \ac{snn}s can be incorporated into lightweight, resource-aware federated protocols. This ensures that a wide range of agents—each operating under different sensing modalities and spiking dynamics—can still benefit from collaborative learning without compromising on latency or energy constraints.
