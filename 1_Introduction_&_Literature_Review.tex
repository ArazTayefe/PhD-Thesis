...
\ac{snn}s represent the third generation of neural models, operating with discrete spikes that carry both temporal and spatial information. Unlike conventional neural networks that rely on continuous activations, \ac{snn}s process asynchronous event streams, providing rich temporal dynamics and exceptionally low power consumption. For instance, neuromorphic implementations have demonstrated energy consumption on the order of tens of picojoules per synaptic operation, compared to nanojoule-level energy costs in conventional GPU-based ANN inference, corresponding to improvements of up to two to three orders of magnitude depending on the workload and hardware platform. These characteristics make \ac{snn}s highly suitable for edge intelligence, real-time robotics, and distributed autonomous systems, where energy efficiency and reaction speed are critical \cite{Ch_1_R2, Ch_1_R3}.
...
\begin{equation} \label{Eq.1}
    R_{m}C_{m}\frac{dV\left(t\right)}{dt} = E_{L} - V\left(t\right) + R_{m}I\left(t\right)
\end{equation}

\noindent where $V(t)$ is the neuron's membrane potential shown as \(V_{1}\) and \(V_{2}\) in Table \ref{Table.SRF}, $R_{m}$ is the membrane resistance, $C_{m}$ is the membrane capacitance, $E_{L}$ is the resting potential, and $I(t)$ is the input current. The neuron spikes when its potential reaches the threshold potential ($V_{th}$). The potential of the neuron immediately reaches the reset potential ($V_{res}$) after it spikes.
...
\noindent where $t_{isi}$ is the inter-spike interval that can be calculated using the neuron model. When the membrane potential reaches the threshold ($V_{th}$), the neuron emits a spike and is reset to $V_{res}$. Therefore, the inter-spike interval can be obtained from the analytical solution of (\ref{Eq.1}) under constant input current.

Starting from (\ref{Eq.1}), the solution of the membrane potential is given by,

\begin{equation}
    V(t) = E_{L} + R_{m}I + \left(V_{res} - E_{L} - R_{m}I\right)e^{-t/\tau_{m}}
\end{equation}

\noindent where $\tau_{m}=R_{m}C_{m}$ is the membrane time constant. The inter-spike interval corresponds to the time required for the membrane potential to evolve from $V_{res}$ to $V_{th}$, i.e.,

\begin{equation}
    V_{th} = E_{L} + R_{m}I + \left(V_{res} - E_{L} - R_{m}I\right)e^{-t_{isi}/\tau_{m}}
\end{equation}

\noindent Rearranging the above equation gives,

\begin{equation}
    V_{th} - E_{L} - R_{m}I = \left(V_{res} - E_{L} - R_{m}I\right)e^{-t_{isi}/\tau_{m}}
\end{equation}

\noindent Dividing both sides by $\left(V_{res} - E_{L} - R_{m}I\right)$ yields,

\begin{equation}
    \frac{V_{th} - E_{L} - R_{m}I}{V_{res} - E_{L} - R_{m}I} = e^{-t_{isi}/\tau_{m}}
\end{equation}

\noindent Taking the natural logarithm of both sides results in,

\begin{equation}
    -\frac{t_{isi}}{\tau_{m}} = \ln\left(\frac{V_{th} - E_{L} - R_{m}I}{V_{res} - E_{L} - R_{m}I}\right)
\end{equation}

\noindent which leads to the inter-spike interval expression,
...
The Izhikevich model is a widely used spiking neuron model that balances biological plausibility with computational efficiency. It captures a variety of neuronal firing patterns observed in real neurons while remaining relatively simple to implement. The model is defined by the following set of differential equations, where the membrane potential \(V\) is expressed in \(\mathrm{mV}\), the recovery variable \(U\) is expressed in \(\mathrm{mV}\), the input current \(I\) is expressed in \(\mathrm{mV/ms}\), and time \(t\) is expressed in \(\mathrm{ms}\) \cite{Ch_1_R14}:
\begin{equation}
    \frac{dV}{dt} = 0.04V^2 + 5V + 140 - U + I
\end{equation}
\begin{equation}
    \frac{dU}{dt} = a(bV - U)
\end{equation}
\noindent where \(V\) (\(\mathrm{mV}\)) represents the membrane potential of the neuron, \(U\) (\(\mathrm{mV}\)) is a recovery variable that accounts for the activation of potassium ionic currents and inactivation of sodium ionic currents, and \(I\) (\(\mathrm{mV/ms}\)) is the input current. The parameters \(a\), \(b\), \(c\), and \(d\) are dimensionless constants that can be adjusted to model different types of neurons. The model also includes a reset mechanism that occurs when the membrane potential \(V\) reaches a certain threshold of \(30~\mathrm{mV}\):
\begin{equation}
    \text{if } V \geq 30 \text{ mV, then } 
    \begin{cases}
        V \leftarrow c \\
...
Hebbian learning describes synaptic adaptation based on correlated activity between pre- and postsynaptic neurons, often summarized as ``neurons that fire together, wire together"~\cite{Ch_1_R16}. In \ac{snn}s, this principle governs local synaptic updates driven by spike interactions.

Several extensions, such as covariance-based updates, Oja's normalization rule, and BCM theory, refine Hebbian learning by introducing stability and activity-dependent thresholds~\cite{Ch_1_R32, Ch_1_R19, Ch_1_R31}.


In modern \ac{snn}s, Hebbian mechanisms are often combined with reward-modulated plasticity, enabling task-driven learning through global feedback signals.
...
In spiking neural systems, multiple encoding schemes can be employed to represent information, including rate coding, phase coding, and amplitude-based representations. In this work, rate coding is adopted due to its inherent robustness and simplicity in mapping stochastic spike trains to continuous variables. Unlike phase coding, which relies on precise spike timing relative to an oscillatory reference, or amplitude-based schemes that require stable analog representations of signal magnitude, rate coding aggregates neural activity over a temporal window and represents information through spike counts. This aggregation reduces sensitivity to noise, temporal jitter, and hardware variability. Furthermore, rate coding enables a direct and stable interface between neural activity and downstream processing units, since the resulting representation is inherently compatible with discrete-time decision processes and control signals. As a result, rate coding provides a practical compromise between biological plausibility, computational tractability, and robustness in real-world implementations.

In spiking neural networks, synaptic influence can be modeled either through amplitude modulation using synaptic weights or through temporal modulation using synaptic delays. In the conventional formulation adopted in this work, synaptic weights scale the post-synaptic current, allowing the membrane potential to integrate incoming spikes in a stable and cumulative manner. In contrast, delay-based representations encode information by shifting the arrival time of spikes, thereby relying on precise temporal coordination across neurons. While delay-based mechanisms can, in principle, capture rich temporal relationships, they introduce a high sensitivity to timing variations and require fine-grained synchronization, which can lead to instability in both inference and learning. Small perturbations in spike timing may result in significant deviations in network behavior, especially in systems with stochastic inputs or discretized simulation time steps.

By comparison, weight-based modulation provides a more robust and tractable framework. Since information is accumulated over time through weighted integration, the system becomes less sensitive to temporal jitter and discretization effects. This property is particularly important in practical implementations, where numerical integration, asynchronous updates, and hardware imperfections introduce unavoidable timing variability. Furthermore, learning rules such as spike-timing-dependent plasticity operate more consistently when synaptic efficacy is adjusted through weight updates rather than delay shifts, as the gradient of influence remains continuous with respect to synaptic strength. From a computational perspective, weight adaptation also enables smoother convergence and avoids the combinatorial complexity associated with optimizing temporal delays. Therefore, the use of synaptic weights in this framework provides a favorable balance between expressiveness, stability, and implementation simplicity, making it more suitable for scalable and reliable spiking neural systems.


The average firing rate and total spike count of neurons are strongly influenced by the underlying neuron model and the temporal resolution of simulation. For the \ac{lif} model, which is typically simulated with a time step on the order of $1$ ms, the firing rate is constrained by the membrane time constant and refractory period, leading to relatively smooth and bounded spike activity. In contrast, the Izhikevich model, often simulated with finer temporal resolution (e.g., $0.1$ ms), is capable of producing richer dynamical behaviors, including bursting and rapid spiking transitions. As a result, the same physical time interval may contain a significantly larger number of spikes in the Izhikevich model compared to the \ac{lif} model. Therefore, when comparing firing rates or spike counts across models, it is essential to account for both the integration time step and the intrinsic neuron dynamics. The measured spike count over a fixed window $\Delta T$ can be expressed as $N = \sum_{t} f(t)$, where $f(t)$ denotes the spike occurrence, and the corresponding firing rate is given by $r = N / \Delta T$. This formulation highlights that both temporal discretization and neuron dynamics jointly determine the effective rate representation.


One of the fundamental advantages of spiking neural networks lies in their potential for energy-efficient computation. Unlike conventional artificial neural networks, where all neurons are typically active at every computational step, spiking neurons communicate only through discrete events, resulting in sparse activity patterns. Energy consumption is therefore primarily associated with spike generation and transmission rather than continuous numerical operations. In event-driven regimes, where input stimuli are sparse or intermittent, the number of active neurons remains low, leading to significant reductions in computational load and power consumption. Additionally, the absence of high-precision floating-point operations further contributes to efficiency, particularly in hardware implementations. This event-driven paradigm enables scalable and low-power processing, making spiking systems well-suited for resource-constrained environments.
