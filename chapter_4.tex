\chapter{Comparison of Cellular Network Controllers for Quadrotors Experiencing Time Delay}
\label{Ch:ch4}
\thispagestyle{empty}
...
\label{sec:PM_des}
	...
	The goal of our paper is to perform a comparative study and investigate the performance of linear and nonlinear controllers in quadrotor control operating using cellular or 5G networks. The controller is run on the Ground Control Station for the following reasons:
	...
	Fig.~\ref{fig:ControlFramework} shows the control framework. The controller sends control commands $U(t)$ to the quadrotor using the cellular networks. The quadrotor receives control commands and sends back position and orientation $S(t)$. The recorded delay history is used for time delay estimation ($\hat{\tau}_B$).
	
	\begin{figure}
		\centerline{\includegraphics{Figures/ControlFramework.pdf}}
	...
	The paper is organized as follows. Section II presents the dynamic model of the quadrotor, and section III provides details about the controllers. Section III is composed of two parts: (a) Linear PD and (b) Backstepping controllers. Section IV discusses the state estimation process based on the dynamic model. Section V presents the delay estimation process, section VI gives details about the numerical simulations and the results, and section VII summarizes the paper.

 \section{Quadrotor dynamic model} %_________________________________________
	
	In this section, the dynamic model of the quadrotor is presented. The quadrotor is a six Degree-of-Freedom (DOF) system controlled by four motors, as shown in Fig.~\ref{fig:QuadrotorModel}. It is an underactuated system that makes the control process a challenging task.
	
	\begin{figure}
		\centerline{\includegraphics{Figures/DroneModel.pdf}}
		\caption{Quadrotor's dynamic model}
		\label{fig:QuadrotorModel}
	\end{figure}
	...
	\subsection{Linear PD Controller} %_________________________________________
	...
	\noindent where, $e(t)$ is the error between the desired state and the current state, and $K_p$, $K_i$, and $K_d$ are PID gains. In this paper, a PD controller is designed to control the rotational and transitional motions. The controller is designed based on the linearized model of the quadrotor.
	...
	\subsection{Backstepping Controller}  % ____________________________________
	...
	% Equations \eqref{BS7} and \eqref{BS8}
	...
	\section{State Estimator} %_________________________________________________
	...
	In this paper, the delay is assumed to be a discrete-time, discrete-space stochastic process. In order to simulate the network delays during simulation, a fixed predefined transition matrix can be used as follows,
	...
	The nonlinear quadrotor model was used for the simulation. The dynamic parameters and state estimator coefficients are shown in Table \ref{Table1.2}.
	...
	% Perfect Est Low ________________________________________________________
	
	\begin{figure}
		\centering
		\centerline{\includegraphics{Figures/RMSEPerfect.eps}}
	...
	% Normal Low ________________________________________________________
	
	\begin{figure}
		\centering
		\centerline{\includegraphics{Figures/RMSENormalLow.eps}}
	...
	\begin{figure}
		\centerline{\includegraphics{Figures/UNormalHigh.eps}}
	...
	% Delay Change ________________________________________________________
	
	
	\begin{figure}
		\centerline{\includegraphics{Figures/RMSEDlyChng.eps}}
	...
	\begin{figure}
		\centerline{\includegraphics{Figures/UDelayChange.eps}}
